...
samp<-"CDKO/Per2-Luc"  ############################################### provide sampe name
file<-"2011-10-16_CDKO$PLIDCG-10_Raw.csv" ################################## data file to read
plotname<-"p_CDKO"   ############################################# plot name
dats<-list()
nrows<-c()
...
# plot wavelet as pdf
pdf("p_CDKO_wt.pdf", width = 4, height = 2)
...
p_clone12_wt<-wt.image(my.w, n.levels = 250, # colors
                       ...
                       # periodtck = 0.02, 
                       periodtcl = -0.2,
                       # spec.time.axis = list(at = seq(0,240*6,24*6), labels = seq(0,240,24)),
...
#     annotate("text", label=samp, color="black", x=max(dat1$Days), y=max(dat1$Luminescence)+0.2, hjust=1, vjust=0.75, size=2) # label/legend for single sample
# }
# assign(plotname,p)
# 
# # output pdf
