...
samp.clih<-"WT/CMV-Luc"  ############################################### provide sampe name
file<-"2013-2-26WT$CLIH-Cont-DH_Raw.csv" ################################## data file to read
plotname<-"p_CLIH"   ############################################# plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  # samp.clih[i]<-substr(files[i], 1, nchar(files[i])-4)	##### sample name;
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.clih)
}else{
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.clih, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2) # label/legend for single sample
}
assign(plotname,p)
