...
samp.2DG<-c("Vehicle", "2DG")  ##################################### provide sampe name
plotname<-"p_2DG"   ############################################ plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.2DG[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.2DG))) +  
...
p<- p + annotate("rect", xmin=1.2, xmax=4, ymin=0.1 , ymax=0.1+rg/25, alpha=0.25, size=0, color="black", fill="black")
assign(plotname,p)

# # output pdf
ggsave(filename=paste(plotname,"pdf",sep="."), plot=get(plotname), useDingbats=F, width=1.25, height=.75)
