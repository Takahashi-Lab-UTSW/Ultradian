...
p <- ggplot(data=df, aes(x=factor(samp, levels=unique(samp)), y=KG, fill=samp)) +  
  geom_dotplot(binaxis="y", stackdir="center") +
  # geom_errorbar(aes(ymin=1.15, ymax=1.3), width=.2, size=0.1, position=position_dodge(.9))+
...
p<- p + annotate("text", x = 2, y = 1.35, label = "**" , color="black", size=2)
