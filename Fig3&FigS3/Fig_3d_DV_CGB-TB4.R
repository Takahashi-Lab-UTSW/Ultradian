...
offs <- 40  # Input offset in Hours ###########################
cnm <- c("TMRM","BeRST","GCaMP")  # Channel/sensor names #########################
setwd(path)

#read data --------
df <- read.csv('20160603CGB-TB01_04_combine.csv')[,-1] %>% 
  mutate(Hours=(row_number()-1)*dt/60) %>%
...
p <- ggplot(data=dat1, aes(x=Hours, y=Intensity, group = Sensor)) +  
  ...
  scale_y_continuous(expand=c(0.,0.1), breaks=pretty_breaks(n = 3), labels=scaleFUN) # set y-axis labels
