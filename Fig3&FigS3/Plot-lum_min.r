path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_B/Fig3/Min+"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.min<-c("Minimal", "+Pyruvate", "+Glutamine","+Glucose","+FBS")  ##################################### provide sampe name
plotname<-"p_MIN"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
# files<-files[grepl("_2DG",files)]
n<-length(samp.min)
...
for (i in 1:n){
  df<-read.csv(files[grepl(samp.min[i],files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  dat0<-sweep(dat[,-1], 2, colMeans(dat[dat[,1]<0.1,-1]), FUN="/") #normalize to mean of the 1st 0.1 day
  dat0<-cbind(dat[,1],dat0)
  colnames(dat0)<-c("Days",samp.min)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.min))) +  
...
# # Add rectangles
# p<- p + annotate("rect", xmin=1.01, xmax=2.01, ymin=0.0 , ymax=0.1, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.min, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
