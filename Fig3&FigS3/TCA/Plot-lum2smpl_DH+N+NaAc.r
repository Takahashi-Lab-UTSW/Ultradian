...
samp.NNaAc<-c("NaCl 5mM", "NaAc 1mM","NaAc 5mM")  ##################################### provide sampe name
...
for (i in 1:n){
  df<-read.csv(files[grepl(samp.NNaAc[i],files, fixed = TRUE)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.NNaAc)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.NNaAc)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.NNaAc))) +  
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.NNaAc, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
