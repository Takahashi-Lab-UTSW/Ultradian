path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_B/Fig3/Com-"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.com<-c("Complete", "-Pyruvate", "-Glutamine","-Glucose","-FBS")  ##################################### provide sampe name
plotname<-"p_COM"   ############################################ plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  df<-read.csv(files[grepl(samp.com[i],files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.com))) +  
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.com, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
