...
samp.KCl<-c("NaCl", "KCl")  ##################################### provide sampe name
plotname<-"p_KCl"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_NaK",files)]    ###############################
n<-length(samp.KCl)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.KCl[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.KCl))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=2.1, xmax=4.9, ymin=0.45 , ymax=0.45+rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.KCl, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
