...
samp.Nimodipine<-c("Vehicle", "Nimodipine")  ##################################### provide sampe name
plotname<-"p_Nimodipine"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_Nimodipine",files)]    ###############################
n<-length(samp.Nimodipine)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.Nimodipine[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.Nimodipine)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.Nimodipine)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.Nimodipine))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=4.5,margin = margin(t=0.2, b=.2, unit = "line")),
  ...
  scale_y_continuous(expand=c(0.1,0.1), limits=c(0.5, 1.5), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
...
# # Add rectangles
p<- p + annotate("rect", xmin=1, xmax=2, ymin=0.5 , ymax=0.5+1/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.Nimodipine, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
