path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_B/Fig3/DH-N+DmKG/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.NKG<-c("Vehicle","Gln 2mM", "DmKG 1mM","DmKG 2mM","DmKG 4mM")  ##################################### provide sampe name
plotname<-"p_NKG"   ############################################ plot name
dats<-list()
nrows<-c()
...
clrs<-c( "black","blue", "orange","violet","red")
c<-clrs[1:n]

# read raw data exported from Lumicycle Analysis
for (i in 1:n){
  df<-read.csv(files[grepl(paste("_",samp.NKG[i],sep=""),files, fixed = TRUE)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  dat0<-sweep(dat[,-1], 2, colMeans(dat[dat[,1]<0.1,-1]), FUN="/") #normalize to mean of the 1st day
  dat0<-cbind(dat[,1],dat0)
  colnames(dat0)<-c("Days",samp.NKG)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.NKG))) +  
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.NKG, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
