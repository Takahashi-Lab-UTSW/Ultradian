...
samp.ouab<-c("Vehicle", "Ouabain")  ##################################### provide sampe name
plotname<-"p_Ouabain"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_Ouabain",files)]    ###############################
n<-length(samp.ouab)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.ouab[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.ouab)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.ouab))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=4.5,margin = margin(t=0.25, b=.25, unit = "line")),
...
# # Add rectangles
p<- p + annotate("rect", xmin=2.35, xmax=5.15, ymin=0 , ymax=rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.ouab, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
