...
samp.NNC<-c("Vehicle", "NNC")  ##################################### provide sampe name
plotname<-"p_NNC"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_NNC",files)]    ###############################
n<-length(samp.NNC)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.NNC[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.NNC))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.3, xmax=2.4, ymin=0.6 , ymax=0.6+rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.NNC, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
