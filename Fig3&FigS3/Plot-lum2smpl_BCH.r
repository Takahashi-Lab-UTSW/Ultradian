...
samp.bch<-c("Vehicle", "BCH")  ##################################### provide sampe name
plotname<-"p_BCH"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_BCH",files)]    ###############################
n<-length(samp.bch)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.bch[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.bch))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.4, xmax=4, ymin=0.6 , ymax=0.62, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.bch, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
