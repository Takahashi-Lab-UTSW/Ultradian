...
p <- ggplot(data=dat1, aes(x=Mins, y=R.I., group = Drug, colour = factor(Drug, labels=samp.TMKG))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=4.5,margin = margin(t=0,b=0, unit = "pt")),
  ...
  scale_y_continuous(expand=c(0.1,0.2), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
