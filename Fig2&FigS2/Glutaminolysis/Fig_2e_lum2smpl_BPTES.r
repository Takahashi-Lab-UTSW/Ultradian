...
samp.bptes<-c("Vehicle", "BPTES")  ##################################### provide sampe name
plotname<-"p_BPTES"   ############################################ plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("_",samp.bptes[i],"_",sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.bptes)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.bptes)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.bptes))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=0.93, xmax=2.15, ymin=0.45 , ymax=0.45+rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.bptes, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
