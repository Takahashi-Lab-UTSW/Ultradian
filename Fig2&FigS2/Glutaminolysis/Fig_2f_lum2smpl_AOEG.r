...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.AOEG))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=4.5,margin = margin(t=.1,b=.1, unit = "line")),
  ...
  scale_y_continuous(expand=c(0.1,0.1), limits=c(0.1,2.4), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
...
# # Add rectangles
p<- p + annotate("rect", xmin=0.7, xmax=2.15, ymin=0.1 , ymax=0.1+2.3/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
