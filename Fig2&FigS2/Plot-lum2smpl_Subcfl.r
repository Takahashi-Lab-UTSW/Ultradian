path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_B/Fig4/Subcfl/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.Subcfl<-c("5000", "25000", "125000")  ##################################### provide sampe name
lbl.subcfl<-paste(samp.Subcfl, "cells")
plotname<-"p_Subcfl"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
# files<-files[grepl("_KCa",files)]    ###############################
n<-length(samp.Subcfl)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.Subcfl[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.Subcfl)
...
if (n>1){
  dat0<-sweep(dat[,-1], 2, colMeans(dat[dat[,1]<0.25,-1]), FUN="/") #normalize to mean of the 1st day
  dat0<-cbind(dat[,1],dat0)
  colnames(dat0)<-c("Days",samp.Subcfl)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=lbl.subcfl))) +  
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.Subcfl, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
