...
samp.DMS<-c("Vehicle", "DMS 1mM","DMS 2mM", "DMS 5mM", "Gln 2mM")  ##################################### provide sampe name
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("_",samp.DMS[i],sep=""),files, fixed = TRUE)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.DMS))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=4.5,margin = margin(t=.15,b=.15, unit = "line")),
  ...
  scale_y_continuous(limits = c(0,1.6), expand=c(0,0), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.DMS, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
