path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_D/Fig2/FigS2_TCA/DH-N+NaAc/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.NaAc<-c("NaCl 5mM", "NaAc 1mM","NaAc 5mM", "Gln 2mM")  ##################################### provide sampe name
plotname<-"p_DH-N+NaAc"   ############################################ plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste(samp.NaAc[i]),files, fixed = TRUE)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.NaAc)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.NaAc))) +  
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.NaAc, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
