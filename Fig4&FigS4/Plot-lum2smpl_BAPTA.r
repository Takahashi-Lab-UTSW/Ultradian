path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_B/Fig4/BAPTA/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.BAPTA<-c("Vehicle", "BAPTA")  ##################################### provide sampe name
plotname<-"p_BAPTA"   ############################################ plot name
dats<-list()
nrows<-c()
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.BAPTA[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.BAPTA)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.BAPTA)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.BAPTA))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1, xmax=3, ymin=0.3 , ymax=0.325, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.BAPTA, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
