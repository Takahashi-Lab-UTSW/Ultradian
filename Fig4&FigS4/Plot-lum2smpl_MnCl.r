...
samp.MnCl<-c("Vehicle", "MnCl")  ##################################### provide sampe name
plotname<-"p_MnCl"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_MnCl",files)]    ###############################
n<-length(samp.MnCl)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.MnCl[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.MnCl)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.MnCl))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.6, xmax=2.8, ymin=0.5 , ymax=0.6, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.MnCl, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
