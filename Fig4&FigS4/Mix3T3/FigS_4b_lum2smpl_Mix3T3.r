path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_D/Fig4/Mix3T3/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.Mix3T3<-c("150K~0", "150K~100K", "150K~250K")  ##################################### provide sampe name
lbl.3T3<-c("150K : 0", "150K : 100K", "150K : 250K")
plotname<-"p_Mix3T3"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
# files<-files[grepl("_KCa",files)]    ###############################
n<-length(samp.Mix3T3)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("_",samp.Mix3T3[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.Mix3T3)
...
#   colnames(dat0)<-c("Days",samp.Mix3T3)
# }else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=lbl.3T3))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=5,margin = margin(t=.3, b=.3, unit = "pt")),
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.Mix3T3, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
