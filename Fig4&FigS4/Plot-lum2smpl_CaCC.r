...
samp.CaCC<-c("Vehicle", "TA", "NPPB", "CaCCinh-A01")  ##################################### provide sampe name
plotname<-"p_CaCC"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_CaCC",files)]    ###############################
n<-length(samp.CaCC)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.CaCC[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.CaCC)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.CaCC))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.22, xmax=2.25, ymin=-0.15 , ymax=0, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.CaCC, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
