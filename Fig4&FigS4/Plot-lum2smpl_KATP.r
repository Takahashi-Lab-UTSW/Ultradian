...
samp.KATP<-c("Vehicle", "Glybenclamide","Diazoxide")  ##################################### provide sampe name
plotname<-"p_KATP"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_KATP",files)]    ###############################
# file0<-files[grepl("Vehicle", files)]
# file<-c(file0,file1)

n<-length(samp.KATP)
# clrs<-c("blue", "orange","green", "red", "violet","cyan") # 6 colours
clrs<-c("blue", "red", "green","violet","cyan")
c<-clrs[1:n]

# read raw data exported from Lumicycle Analysis
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.KATP[i],sep=""),files, fixed = TRUE)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.KATP)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.KATP))) +  
  ...
  scale_y_continuous(limits = c(0.5,2), expand=c(0,0), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.KATP, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
