...
samp.Eosin<-c("Vehicle", "Eosin")  ##################################### provide sampe name
plotname<-"p_Eosin"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_Eosin",files)]    ###############################
n<-length(samp.Eosin)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.Eosin[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.Eosin)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.Eosin)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.Eosin))) +  
  ...
  scale_y_continuous(limits = c(0.5,1.5), expand=c(0.1,0), breaks=pretty_breaks(n = 4), labels=scaleFUN) # set y-axis labels
...
# # Add rectangles
p<- p + annotate("rect", xmin=2, xmax=6, ymin=0.5 , ymax=0.525, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.Eosin, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
