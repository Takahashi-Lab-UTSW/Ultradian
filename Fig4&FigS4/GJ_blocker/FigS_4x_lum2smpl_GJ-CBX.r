path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_D/Fig4/GJ_blocker/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.GJCBX<-c("Glycyrrhizin", "Carbenoxolone")  ##################################### provide sampe name
plotname<-"p_GJCBX"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_GJ",files)]    ###############################
n<-length(samp.GJCBX)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.GJCBX[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.GJCBX)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.GJCBX)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.GJCBX))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.15, xmax=4, ymin=-0.05 , ymax=-0.05+rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.GJCBX, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)

# # output pdf
ggsave(filename=paste(plotname,"pdf",sep="."), plot=get(plotname), useDingbats=F, width=1.5, height=0.75)
