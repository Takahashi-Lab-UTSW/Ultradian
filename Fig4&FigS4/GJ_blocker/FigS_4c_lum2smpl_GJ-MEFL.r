...
samp.GJMEFL<-c("Chloroquine", "Mefloquine")  ##################################### provide sampe name
plotname<-"p_GJMEFL"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
files<-files[grepl("_GJ",files)]    ###############################
n<-length(samp.GJMEFL)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("-",samp.GJMEFL[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]
...
colnames(dat)<-c("Days",samp.GJMEFL)
...
if (n>1){
  ...
  colnames(dat0)<-c("Days",samp.GJMEFL)
}else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=samp.GJMEFL))) +  
...
# # Add rectangles
p<- p + annotate("rect", xmin=1.15, xmax=4, ymin=0.4 , ymax=0.4+rg/25, alpha=0.25, size=0, color="black", fill="black")
# # Add segments
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.GJMEFL, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.5) # label/legend for single sample
}
assign(plotname,p)
