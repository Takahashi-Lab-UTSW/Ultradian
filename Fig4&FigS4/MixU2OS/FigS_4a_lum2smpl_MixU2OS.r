# luminescence /1000 for plot purpose

path<-"C:/Users/SHU/Documents/Doc/UltradianCycle/UC-Plots/All_Figs/Figs_D/Fig4/MixU2OS/"

setwd(paste(path))

# folders = list.dirs('.', full.names=FALSE, recursive=FALSE)
# read csv data exported from Lumicycle Analysis
samp.MixU2OS<-c("0~250K", "150K~100K", "150K~250K")  ##################################### provide sampe name
lbl.U2OS<-c("0 : 250K", "150K : 100K", "150K : 250K")
plotname<-"p_MixU2OS"   ############################################ plot name
dats<-list()
nrows<-c()
files = list.files(pattern="*.csv")
# files<-files[grepl("_KCa",files)]    ###############################
n<-length(samp.MixU2OS)
...
for (i in 1:n){
  df<-read.csv(files[grepl(paste("_",samp.MixU2OS[i],sep=""),files)], skip=3, col.names = c("Date","Time",	"Days",	"CountsPerSec",	"Baseline")) 
  lum<-df[,"CountsPerSec"]/1000   # devide lum by 1000
...
colnames(dat)<-c("Days",samp.MixU2OS)
...
#   colnames(dat0)<-c("Days",samp.MixU2OS)
# }else{
...
p <- ggplot(data=dat1, aes(x=Days, y=Luminescence, group = Sample, colour = factor(Sample, labels=lbl.U2OS))) +  
  ...
  theme(axis.title = element_text(face = "plain", color = "black", size = 6), 
        ...
        legend.text = element_text(size=5,margin = margin(t=.25, b=.25, unit = "pt")),
...
if (n==1){
  p<-p + theme(legend.position="none") + 
    annotate("text", label=samp.MixU2OS, color="black", x=max(dat1$Days), y=max(dat1$Luminescence), hjust=1, vjust=0.75, size=2.8) # label/legend for single sample
}
assign(plotname,p)
